...
const generateMessageIDV2 = (userId) => {
  ...
  const randomHexLength = totalLength - prefix.length; // 24
  const randomPart = hash
    .toString('hex')
    .toUpperCase()
    .substring(0, randomHexLength);

  const position = Math.floor(Math.random() * 3); 
  // 0 = depan, 1 = tengah, 2 = belakang
  ...
  if (position === 1) {
    ...
    return (
      randomPart.slice(0, middleIndex) +
      prefix +
      randomPart.slice(middleIndex)
    );
  }

  return randomPart + prefix;
};
...
const generateMessageID = () => {
  const prefix = 'ISMEELYN';
  const totalLength = 32;
  const randomHexLength = totalLength - prefix.length;

  const randomPart = crypto_1
    .randomBytes(randomHexLength / 2)
    .toString('hex')
    .toUpperCase();
