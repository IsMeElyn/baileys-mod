...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, pairKey = "ISMEELYN") => { 
if (pairKey && typeof pairKey === "string") {
    let cleaned = pairKey.replace(/[^a-zA-Z0-9]/g, "").toUpperCase();
    ...
    if (cleaned.length === 0) {
        ...
        if (cleaned.length < 8) {
            const needed = 8 - cleaned.length;
            const randomFill = crypto_1.randomBytes(needed)
                .toString("base64")
                .replace(/[^a-zA-Z0-9]/g, "")
                .toUpperCase()
                .slice(0, needed);
        ...
        cleaned = cleaned.slice(0, 8).toUpperCase();
        ...
        if (roll < 0.1) {
            ...
            // 1%
            authState.creds.pairingCode = "ILUVELYN";
        } else if (roll < 11.1) {
            // 10%
            authState.creds.pairingCode = "ISMEELYN";
        } else {
            authState.creds.pairingCode = cleaned;
        }
    }
